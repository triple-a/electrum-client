...
export class ElectrumClient {
  ...
  private consensusChangedListeners = new Map<
    Handle,
    ConsensusChangedListener
  >();
  private headChangedListeners = new Map<Handle, HeadChangedListener>();
  private transactionListeners = new Map<
    Handle,
    { listener: TransactionListener; addresses: Set<string> }
  >();
  private listenerId: Handle = 0;

  private transactionsWaitingForConfirmation = new Map<
    number,
  ...
  public async getTransactionReceiptsByAddress(
    address: string
  ): Promise<Receipt[]> {
    for (const agent of this.agents) {
      try {
  ...
  public async getTransactionsByAddress(
    ...
    if (sinceBlockHeight > 0) {
      const firstUnwantedHistoryIndex = history.findIndex(
        (receipt) =>
          receipt.blockHeight > 0 && receipt.blockHeight < sinceBlockHeight
      );
    ...
    for (const { transactionHash, blockHeight } of history) {
      ...
      if (
        knownTx &&
        knownTx.blockHeight === Math.max(blockHeight, 0) &&
        knownTx.state === TransactionState.CONFIRMED
      ) {
        continue;
      }
      ...
      try {
        ...
        try {
          ...
          if (block) {
            ...
            const confirmed =
              confirmations >= this.options.requiredBlockConfirmations;
            state = confirmed
    ...
    for (const details of knownTxs.values()) {
      if (
        (details.state === TransactionState.NEW ||
          details.state === TransactionState.PENDING) &&
        !txs.some((tx) => tx.transactionHash === details.transactionHash)
      ) {
        // Re-send this transaction
        txs.push(
          await this.sendTransaction(transactionFromPlain(details).toHex())
        );
      }
    }
  ...
  public async sendTransaction(
    ...
    if (tx.onChain) {
      ...
      const blockHeight = receipts.find(
        (receipt) => receipt.transactionHash === tx!.transactionHash
      )!.blockHeight;
  ...
  public async estimateFees(targetBlocks = [25, 10, 5, 2]): Promise<{
    ...
    for (const target of targetBlocks) {
      ...
      const feesForTarget = estimates
        .map((estimate) => estimate[i])
  ...
  public addConsensusChangedListener(
  ...
  public addTransactionListener(
    ...
    const listenerId = this.listenerId++;
    this.transactionListeners.set(listenerId, {
      listener,
  ...
  private async connect(): Promise<void> {
    ...
    for (const transport of [Transport.WSS, Transport.SSL, Transport.TCP]) {
      peers = [...this.addressBook.values()].filter((peer) => {
        const protocol = [null, "tcp", "ssl", "wss"][transport] as
          | "tcp"
          | "ssl"
          | "wss";
    ...
    const agentOptions: ElectrumAgentOptions | undefined = this.options
      .websocketProxy
      ? {
    ...
    agent.on(AgentEvent.SYNCING, () =>
      this.onConsensusChanged(ConsensusState.SYNCING)
    );
    agent.on(AgentEvent.SYNCED, () => {
      this.agents.add(agent);
      this.onConsensusChanged(ConsensusState.ESTABLISHED);
    });
    agent.on(AgentEvent.BLOCK, (block: PlainBlockHeader) =>
      this.onHeadChanged(block, "extended", [], [block])
    );
    agent.on(AgentEvent.TRANSACTION_ADDED, (tx: PlainTransaction) =>
      this.onPendingTransaction(tx)
    );
    agent.on(
      AgentEvent.TRANSACTION_MINED,
      (tx: PlainTransaction, block: PlainBlockHeader) =>
        this.onMinedTransaction(block, tx, block)
    );
    agent.on(AgentEvent.CLOSE, (reason: string) =>
      this.onConsensusFailed(agent, reason)
    );
    ...
    try {
      ...
      agent.close((error as Error).message);
      return;
    }
  ...
  private addPeers(peers: Peer[]): void {
    ...
    peers = peers.filter((peer) => {
      ...
      if (
        peer.ports.ssl &&
        peer.ports.ssl !==
          (GenesisConfig.NETWORK_NAME === Network.MAIN ? 50002 : 60002)
      )
        return false;
      if (
        peer.ports.tcp &&
        peer.ports.tcp !==
          (GenesisConfig.NETWORK_NAME === Network.MAIN ? 50001 : 60001)
      )
        return false;
  ...
  private queueTransactionForConfirmation(tx: TransactionDetails): void {
    ...
    const map =
      this.transactionsWaitingForConfirmation.get(confirmationHeight) ||
      new Map<string, TransactionDetails>();
  ...
  private clearTransactionFromConfirm(tx: PlainTransaction): void {
    for (const [
      key,
      value,
    ] of this.transactionsWaitingForConfirmation.entries()) {
  ...
  private onConsensusFailed(agent: Agent, _reason: string): void {
    if (agent) {
  ...
  private onHeadChanged(
    block: PlainBlockHeader,
    reason: string,
    revertedBlocks: PlainBlockHeader[],
    adoptedBlocks: PlainBlockHeader[]
  ): void {
    ...
    if (
      this.consensusState === ConsensusState.ESTABLISHED &&
      (!previousBlock || block.blockHash !== previousBlock.blockHash)
    ) {
      for (const listener of this.headChangedListeners.values()) {
        listener(
          block,
          reason,
          revertedBlocks.map((b) => b.blockHash),
          adoptedBlocks.map((b) => b.blockHash)
        );
      }
    }
    ...
    if (this.transactionListeners.size > 0) {
      ...
      for (const block of revertedBlocks) {
        const confirmationHeight = this.getConfirmationHeight(
          block.blockHeight
        );
        const map =
          this.transactionsWaitingForConfirmation.get(confirmationHeight);
      ...
      for (const block of adoptedBlocks) {
        const map = this.transactionsWaitingForConfirmation.get(
          block.blockHeight
        );
        if (map) {
          for (const tx of map.values()) {
            this.onConfirmedTransaction(
              tx,
              adoptedBlocks[adoptedBlocks.length - 1]
            );
          }
  ...
  private getListenersForTransaction(tx: PlainTransaction): {
    ...
    return [...this.transactionListeners.values()].filter(
      ({ addresses }) =>
        tx.inputs.some(
          (input) => input.address && addresses.has(input.address)
        ) ||
        tx.outputs.some(
          (output) => output.address && addresses.has(output.address)
        )
    );
  }
}
