...
export class ElectrumApi {
  ...
  constructor(
    options: Omit<ElectrumApiOptions, "network"> & {
  ...
  public async waitForConnectionEstablished() {
    ...
    return new Promise((resolve, reject) => {
      this.socket.once(
        ElectrumWSEvent.CONNECTED,
        () => (resolve(true), (reject = () => {}))
      );
      this.socket.once(
        ElectrumWSEvent.CLOSE,
        () => (
          reject(new Error("Unable to establish a WebSocket connection")),
          (resolve = () => {})
        )
      );
    });
  ...
  public async getReceipts(addressOrScriptHash: string): Promise<Receipt[]> {
    ...
    receipts.sort(
      (a, b) =>
        (Math.max(0, b.height) || Number.MAX_SAFE_INTEGER) -
        (Math.max(0, a.height) || Number.MAX_SAFE_INTEGER)
    );
  ...
  public async getTransaction(
    ...
    const raw: string = await this.socket.request(
  ...
  public async proofTransaction(
    hash: string,
    block: PlainBlockHeader
  ): Promise<boolean> {
    const transactionMerkleRoot = await this.getTransactionMerkleRoot(
      hash,
      block.blockHeight
    );
    if (transactionMerkleRoot !== block.merkleRoot) {
      throw new Error(
        `Invalid transaction merkle proof for block height: ${hash}, ${block.blockHeight}`
      );
    }
    return true;
  }

  public async getTransactionMerkleRoot(
    hash: string,
    height: number
  ): Promise<string> {
    type MerkleProof = {
    ...
    const proof: MerkleProof = await this.socket.request(
    ...
    if (proof.block_height !== height) {
      throw new Error(
        "Invalid reference block height received in transaction merkle proof"
      );
    }
    ...
    for (const pairHash of proof.merkle) {
      ...
      node = new Uint8Array(
        await subtle.digest(
          "SHA-256",
          await subtle.digest("SHA-256", concatenated)
        )
      );
  ...
  public async estimateFee(targetBlocks: number): Promise<number> {
    const coinsPerKilobyte = await this.socket.request(
  ...
  public async broadcastTransaction(rawTx: string): Promise<PlainTransaction> {
    ...
    let hash: string;
    try {
      hash = await this.socket.request(
        "blockchain.transaction.broadcast",
        rawTx
      );
    } catch (error) {
      if (
        (error as Error).message.includes("Transaction already in block chain")
      ) {
        tx.onChain = true;
        return tx;
      } else throw error;
  ...
  public async subscribeReceipts(
    address: string,
    callback: (receipts: Receipt[]) => any
  ) {
  ...
  public async subscribeHeaders(callback: (header: PlainBlockHeader) => any) {
    return this.socket.subscribe(
      "blockchain.headers",
      (headerInfo: { height: number; hex: string }) => {
        callback(blockHeaderToPlain(headerInfo.hex, headerInfo.height));
      }
    );
  }

  public async setProtocolVersion(
    clientName: string,
  ...
  public async getPeers(): Promise<Peer[]> {
    const peers: Array<[string, string, string[]]> = await this.socket.request(
