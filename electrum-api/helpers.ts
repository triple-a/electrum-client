...
export function blockHeaderToPlain(
  header: string | BitcoinJS.Block,
  height: number
): PlainBlockHeader {
  ...
  return {
    ...
    prevHash: header.prevHash
      ? bytesToHex(new Uint8Array(header.prevHash).reverse())
      : null,
    merkleRoot: header.merkleRoot
      ? bytesToHex(new Uint8Array(header.merkleRoot).reverse())
      : null,
  };
}

export function transactionToPlain(
  tx: string | BitcoinJS.Transaction,
  network: BitcoinJS.Network
): PlainTransaction {
  if (typeof tx === "string") tx = BitcoinJS.Transaction.fromHex(tx);

  const inputs = tx.ins.map((input: BitcoinJS.TxInput, index: number) =>
    inputToPlain(input, index, network)
  );
  const outputs = tx.outs.map((output: BitcoinJS.TxOutput, index: number) =>
    outputToPlain(output, index, network)
  );
...
export function outputToPlain(
  output: BitcoinJS.TxOutput,
  index: number,
  network: BitcoinJS.Network
): PlainOutput {
  let address: string | null = null;
...
export function deriveAddressFromInput(
  input: BitcoinJS.TxInput,
  network: BitcoinJS.Network
): string | undefined {
  ...
  if (chunks.length > 2 && witness.length === 0) {
    ...
    if (
      ...
      BitcoinJS.script.OPS.OP_CHECKMULTISIG
    ) {
      const m = chunks.length - 2; // Number of signatures
      const pubkeys = redeemScript.filter(
        (n) => typeof n !== "number"
      ) as Buffer[];
  ...
  if (chunks.length === 1 && witness.length > 2) {
    const redeemScript = BitcoinJS.script.decompile(
    ...
    if (
      ...
      const pubkeys = BitcoinJS.script
        .decompile(witness[witness.length - 1])!
    ...
    if (
      witness.length === 3 &&
      redeemScript.filter((n) => typeof n !== "number").length === 1
    ) {
      return BitcoinJS.payments.p2sh({
        redeem: BitcoinJS.payments.p2wsh({
...
export function transactionFromPlain(
  ...
  tx.ins = plain.inputs
    ...
    .map((input) => inputFromPlain(input));
  tx.outs = plain.outputs
    .sort((a, b) => a.index - b.index)
    .map((output) => outputFromPlain(output));
  return tx;
}
...
export function inputFromPlain(plain: PlainInput): BitcoinJS.TxInput {
  return {
    ...
    witness: plain.witness.map((scriptOrNumber) =>
      typeof scriptOrNumber === "string"
        ? Buffer.from(hexToBytes(scriptOrNumber))
        : (scriptOrNumber as any as Buffer)
    ),
  };
}
